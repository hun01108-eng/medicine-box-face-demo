...
from .templates import FaceProfile, TemplateStore
...
def exit_code(status: IdentityStatus) -> int:
    return {
        IdentityStatus.MATCHED: 0,
        IdentityStatus.UNKNOWN: 2,
        IdentityStatus.RETRY: 3,
    }.get(status, 4)


def is_terminal_result(result: IdentityResult) -> bool:
    if result.status in {IdentityStatus.MATCHED, IdentityStatus.UNKNOWN}:
        return True
    return result.status == IdentityStatus.RETRY and result.reason in {
        "multiple_faces",
        "timeout",
    }
...
def simulate(case: str, config: AppConfig) -> int:
    ...
    cases = {
        "known": [(0.0, 1, True, 0.68, ""), (0.2, 1, True, 0.64, ""), (0.4, 1, True, 0.66, "")],
        "unknown": [
            (index * 0.2, 1, True, score, "")
            for index, score in enumerate([0.22, 0.31, 0.36, 0.29, 0.40])
        ],
        "dark": [(0.0, 1, False, None, "too_dark")],
        "multiple": [(0.0, 2, True, None, "multiple_faces")],
        "timeout": [(0.0, 1, True, 0.60, ""), (3.1, 1, True, 0.60, "")],
    }
    result = None
    for values in cases[case]:
        result = decision.update(FrameObservation(*values))
...
def load_profile(config: AppConfig, root: Path) -> FaceProfile:
    profile = TemplateStore(root / config.profile_path).load()
    if profile.user_id != config.decision.user_id:
        raise ValueError(
            "profile user_id does not match configured decision user_id: "
            f"{profile.user_id!r} != {config.decision.user_id!r}"
        )
    return profile
    ...
    try:
        for frame in camera.frames():
            ...
            if display:
                ...
                label = f"{result.status.value} score={confidence}"
...
class OverlayRenderer:
    ...
    def put_text(self, frame, text: str, origin: tuple[int, int], color, height: int = 24) -> None:
        ...
        self.cv2.putText(frame, ascii_text, origin, self.cv2.FONT_HERSHEY_SIMPLEX, height / 32.0, color, 2, self.cv2.LINE_AA)
...
def self_check(config: AppConfig, root: Path) -> int:
    ...
    required = ["face_detector_api", "face_recognizer_api", "detector_model", "recognizer_model", "profile"]
    ready = all(checks.get(key, False) for key in required)
...
def main() -> int:
    ...
    try:
        ...
        if arguments.command == "run":
            return run_live(arguments.source or config.camera_source, arguments.device or config.camera_device, arguments.display, config, root)
        if arguments.command == "monitor":
            return monitor_live(arguments.source or config.camera_source, arguments.device or config.camera_device, config, root)
        if arguments.command == "benchmark":
            return benchmark_live(arguments.source or config.camera_source, arguments.device or config.camera_device, arguments.trials, config, root)
